...
const trimTopLanguages = (
  ...
  const langsToHide: Record<string, true> = {};
...
interface CreateProgressTextNodeProps {
  width: number;
  color: string;
  name: string;
  size: number;
...
const createProgressTextNode = ({
  ...
  statsFormat,
  index,
}: CreateProgressTextNodeProps): string => {
...
interface CreateCompactLangNodeProps {
  lang: Lang;
  totalSize: number;
...
const createCompactLangNode = ({
  ...
  index,
}: CreateCompactLangNodeProps): string => {
...
interface CreateLanguageTextNodeProps {
  langs: Lang[];
  totalSize: number;
  hideProgress?: boolean;
  statsFormat?: string;
...
const createLanguageTextNode = ({
  ...
  hideProgress,
  statsFormat,
}: CreateLanguageTextNodeProps): string => {
  const longestLang = getLongestLang(langs);
  const chunked = chunkArray(langs, langs.length / 2);
  const layouts = chunked.map((array: Lang[]) => {
    const items = array.map((lang: Lang, index: number) =>
...
interface CreateDonutLanguagesNodeProps {
  langs: Lang[];
  totalSize: number;
  statsFormat: string;
}
...
const createDonutLanguagesNode = ({
  ...
  totalSize,
  statsFormat,
}: CreateDonutLanguagesNodeProps): string => {
  return flexLayout({
    items: langs.map((lang: Lang, index: number) => {
...
const createDonutPaths = (
  ...
  for (let i = 0; i < percentages.length; i++) {
    const tmpPath: DonutPath = { d: "", percent: 0 };
...
const renderDonutLayout = (
  ...
  const colors = langs.map((lang: Lang) => lang.color);
  const langsPercents = langs.map((lang: Lang) =>
  ...
  const donutPaths =
    langs.length === 1
      ...
      : langPaths
          .map((section: DonutPath, index: number) => {
...
interface NoLanguagesDataNodeProps {
  color: string;
  text: string;
  layout?: Layout;
}
...
const noLanguagesDataNode = ({
  ...
  text,
  layout,
}: NoLanguagesDataNodeProps): string => {
  return `
    <text x="${
...
interface GetDefaultLanguagesCountByLayoutProps {
  layout?: Layout;
...
const getDefaultLanguagesCountByLayout = ({
  layout,
  hide_progress,
}: GetDefaultLanguagesCountByLayoutProps): number => {
