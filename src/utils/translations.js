...
const statCardLocales = ({ name, apostrophe }) => {
  ...
  return {
  "statcard.title": {
      en: `${encodedName}'${apostrophe} GitLab Stats`,
      ar: `${encodedName} إحصائيات جيت هاب`,
      az: `${encodedName}'${apostrophe} Hesabının GitLab Statistikası`,
      ca: `Estadístiques de GitLab de ${encodedName}`,
      cn: `${encodedName} 的 GitLab 统计数据`,
      "zh-tw": `${encodedName} 的 GitLab 統計資料`,
      cs: `GitLab statistiky uživatele ${encodedName}`,
      de: `${encodedName + apostrophe} GitLab-Statistiken`,
      sw: `GitLab Stats za ${encodedName}`,
      ur: `${encodedName} کے گٹ ہب کے اعداد و شمار`,
      bg: `GitLab статистика на потребител ${encodedName}`,
      bn: `${encodedName} এর GitLab পরিসংখ্যান`,
      es: `Estadísticas de GitLab de ${encodedName}`,
      fa: `آمار گیت‌هاب ${encodedName}`,
      fi: `${encodedName}:n GitLab-tilastot`,
      fr: `Statistiques GitLab de ${encodedName}`,
      hi: `${encodedName} के GitLab आँकड़े`,
      sa: `${encodedName} इत्यस्य GitLab सांख्यिकी`,
      hu: `${encodedName} GitLab statisztika`,
      it: `Statistiche GitLab di ${encodedName}`,
      ja: `${encodedName}の GitLab 統計`,
      kr: `${encodedName}의 GitLab 통계`,
      nl: `${encodedName}'${apostrophe} GitLab-statistieken`,
      "pt-pt": `Estatísticas do GitLab de ${encodedName}`,
      "pt-br": `Estatísticas do GitLab de ${encodedName}`,
      np: `${encodedName}'${apostrophe} गिटहब तथ्याङ्क`,
      el: `Στατιστικά GitLab του ${encodedName}`,
      ro: `Statisticile GitLab ale lui ${encodedName}`,
      ru: `Статистика GitLab пользователя ${encodedName}`,
      "uk-ua": `Статистика GitLab користувача ${encodedName}`,
      id: `Statistik GitLab ${encodedName}`,
      ml: `${encodedName}'${apostrophe} ഗിറ്റ്ഹബ് സ്ഥിതിവിവരക്കണക്കുകൾ`,
      my: `${encodedName} ရဲ့ GitLab အခြေအနေများ`,
      ta: `${encodedName} கிட்ஹப் புள்ளிவிவரங்கள்`,
      sk: `GitLab štatistiky používateľa ${encodedName}`,
      tr: `${encodedName} Hesabının GitLab İstatistikleri`,
      pl: `Statystyki GitLab użytkownika ${encodedName}`,
      uz: `${encodedName}ning GitLab'dagi statistikasi`,
      vi: `Thống Kê GitLab ${encodedName}`,
      se: `GitLabstatistik för ${encodedName}`,
      he: `סטטיסטיקות הגיטהאב של ${encodedName}`,
      fil: `Mga Stats ng GitLab ni ${encodedName}`,
      th: `สถิติ GitLab ของ ${encodedName}`,
      sr: `GitLab статистика корисника ${encodedName}`,
      "sr-latn": `GitLab statistika korisnika ${encodedName}`,
      no: `GitLab-statistikk for ${encodedName}`,
    },
    "statcard.ranktitle": {
      en: `${encodedName}'${apostrophe} GitLab Rank`,
      ar: `${encodedName} إحصائيات جيت هاب`,
      az: `${encodedName}'${apostrophe} Hesabının GitLab Statistikası`,
      ca: `Estadístiques de GitLab de ${encodedName}`,
      cn: `${encodedName} 的 GitLab 统计数据`,
      ...
      cs: `GitLab statistiky uživatele ${encodedName}`,
      de: `${encodedName + apostrophe} GitLab-Statistiken`,
      sw: `GitLab Rank ya ${encodedName}`,
      ur: `${encodedName} کی گٹ ہب رینک`,
      bg: `GitLab ранг на ${encodedName}`,
      bn: `${encodedName} এর GitLab পরিসংখ্যান`,
      es: `Estadísticas de GitLab de ${encodedName}`,
      fa: `رتبه گیت‌هاب ${encodedName}`,
      fi: `${encodedName}:n GitLab-sijoitus`,
      fr: `Statistiques GitLab de ${encodedName}`,
      hi: `${encodedName} का GitLab स्थान`,
      sa: `${encodedName} इत्यस्य GitLab स्थानम्`,
      hu: `${encodedName} GitLab statisztika`,
      it: `Statistiche GitLab di ${encodedName}`,
      ja: `${encodedName} の GitLab ランク`,
      kr: `${encodedName}의 GitLab 통계`,
      nl: `${encodedName}'${apostrophe} GitLab-statistieken`,
      "pt-pt": `Estatísticas do GitLab de ${encodedName}`,
      "pt-br": `Estatísticas do GitLab de ${encodedName}`,
      np: `${encodedName}'${apostrophe} गिटहब तथ्याङ्क`,
      el: `Στατιστικά GitLab του ${encodedName}`,
      ro: `Rankul GitLab al lui ${encodedName}`,
      ru: `Рейтинг GitLab пользователя ${encodedName}`,
      "uk-ua": `Рейтинг GitLab користувача ${encodedName}`,
      id: `Statistik GitLab ${encodedName}`,
      ml: `${encodedName}'${apostrophe} ഗിറ്റ്ഹബ് സ്ഥിതിവിവരക്കണക്കുകൾ`,
      my: `${encodedName} ရဲ့ GitLab အဆင့်`,
      ta: `${encodedName} கிட்ஹப் தரவரிசை`,
      sk: `GitLab štatistiky používateľa ${encodedName}`,
      tr: `${encodedName} Hesabının GitLab Yıldızları`,
      pl: `Statystyki GitLab użytkownika ${encodedName}`,
      uz: `${encodedName}ning GitLab'dagi statistikasi`,
      vi: `Thống Kê GitLab ${encodedName}`,
      se: `GitLabstatistik för ${encodedName}`,
      he: `דרגת הגיטהאב של ${encodedName}`,
      fil: `Ranggo ng GitLab ni ${encodedName}`,
      th: `อันดับ GitLab ของ ${encodedName}`,
