...
You can look at a preview for [all available themes](.github/READ_MES/THEMES.md) or checkout the [theme config file](src/themes/index.js). Please note that we paused the addition of new themes to decrease maintenance efforts; all pull requests related to new themes will be closed.
...
You can use the `bg_color` parameter to make any of [the available themes](.github/READ_MES/THEMES.md) transparent. This is done by setting the `bg_color` to a color with a transparent alpha channel (i.e. `bg_color=00000000`):
...
| `theme` | Name of the theme, choose from [all available themes](.github/READ_MES/THEMES.md). | enum | `default` |
...
If we don't support your language, please consider contributing! You can find more information about how to do it in our [contributing guidelines](.github/READ_MES/CONTRIBUTING.md#translations-contribution).
